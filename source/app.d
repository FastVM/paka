...
void main(string[] args)
{
    ...
    auto info = getopt(args, "repl", &repl, "eval", &stmts, "file",
            &scripts, "caarry", &carry, "world", &world);
    ...
    if (repl)
    {
        ...
        foreach (i; 0 .. states["length"].str.to!size_t)
        {
            linenoiseHistoryAdd(cast(char*) states[i.to!string].object["input"].str.toStringz);
        }
        ...
        while (true)
        {
            ...
            if (code.startsWith("/goto "))
            {
                ...
                File f = File(world, "w");
                f.write(SerialValue(states).serialToString);
                size_t n = states["length"].str.to!size_t - 1;
                rootBase.length = 0;
                loadState(states[n.to!string].object["state"]);
                continue;
