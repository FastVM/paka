...
//     this(size_t alloc)
//     {
//         v = cast(Dynamic*) GC.malloc(alloc);
//         size = alloc;
//         synchronized
//         {
//             size = size + mysize;
...
Cont runMore(T...)(ushort arg, Cont retcont, Function func, Dynamic[] args, T rest,
        Dynamic* stack, Dynamic* locals)
{
    ubyte* instrs = func.instrs.ptr;

    void morecont(Dynamic push)
    {
        ushort index = arg;
        (*(stack - 1)) = push;
        while (true)
        {
            ...
            case Opcode.call:
                ...
                Dynamic f = (*(stack - 1));
                Cont cc = runMore(index, retcont, func, args, rest, stack, locals);
                switch (f.type)
                {
                case Dynamic.Type.fun:
                    f.fun.fun(cc, stack[0 .. 0 + instrs.get1!ushort(index)]);
                    return;
                case Dynamic.Type.del:
                    (*f.fun.del)(cc, stack[0 .. 0 + instrs.get1!ushort(index)]);
                    return;
                case Dynamic.Type.pro:
                    run(cc, f.fun.pro, stack[0 .. 0 + instrs.get1!ushort(index)]);
                    return;
                default:
    ...
    Cont cc = asCont(&morecont);
    return cc;
}

void run(T...)(RawCont rc, Function func, Dynamic[] args, T rest)
{
    run!T(rc.asCont, func, args, rest);
}
...
void run(T...)(Cont retcont, Function func, Dynamic[] args, T rest)
{
    ...
    // LocalMemoryDebug dlocals = new LocalMemoryDebug((func.stab.byPlace.length + 1 + args.length) * Dynamic.sizeof);
    // LocalMemoryDebug dstack = new LocalMemoryDebug((func.stackSize + 1) * Dynamic.sizeof);
    // ref Dynamic* locals() {
    //     return dlocals.v;
    // }
    // ref Dynamic* stack() {
    ...
    runMore(0, retcont, func, args, rest, stack, locals)(Dynamic.nil);
}
