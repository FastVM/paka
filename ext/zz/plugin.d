...
enum special = ["+", "-", "*", "/", "%", "&&", "||", "<", ">", "<=", ">=", "==", "!=", "do", "if", "while", "lambda", "return", "array", "def", "set", "var", "index", "call", "args"];
enum ops = ["+", "-", "*", "/", "%"];
...
Node walk(Node src) {
	if (Form form = cast(Form) src) {
		if (form.form == "zz:single") {
			string name;
			foreach (i, arg; form.args) {
				if (i != 0) {
					name ~= '\n';
				}
				name ~= arg.walk.to!string;
			}
			return new Value(name);
		} else {
			Form ret = new Form(form.form);
